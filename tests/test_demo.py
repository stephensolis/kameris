...
def check_classify_model(model, tempdir):
...
def test_train_model(shared_tempdir):
    ...
    with chdir(shared_tempdir):
        ...
        os.mkdir('output')
        args = dotmap.DotMap({
            'job_file': os.path.join(root_dir, 'demo', 'hiv1-lanl-small.yml'),
...
def test_classify_new_model(shared_tempdir):
    model_filename = os.path.join(
        ...
        'classification-kmers_linear-svm.mm-model'
    )
    check_classify_model(model_filename, shared_tempdir)
